...
async function getProjectDataEligibleForLinking(
  projectId: string,
  parameter: any
): Promise<DataPagedList> {
  // Generate axios parameter
  const ProjectParamCreator = ProjectDataApiAxiosParamCreator();
  const getProjectsParam = await ProjectParamCreator.getDataEligibleForLinking(
    projectId
  );
  ...
  // Calling axios
  const axiosDataEligibleForLinking = await RunAxios(getProjectsParam);
  return axiosDataEligibleForLinking.data;
}
...
function ProjectDataEligibleForLinkingPage() {
  const [
    projectDataEligibleForLinkingResponse,
    setProjectDataEligibleForLinkingResponse,
  ...
  useEffect(() => {
    ...
    async function fetchDataEligibleForLinking() {
      if (projectId) {
        try {
          const data = await getProjectDataEligibleForLinking(
            projectId,
            apiParameter
          );
          if (cancel) return;

          setProjectDataEligibleForLinkingResponse(data);
  ...
  return (
    ...
    >
      <Grid item xs={12}>
        <Typography variant="h4">Project DataEligibleForLinking</Typography>
      </Grid>
      <Grid item xs={12}>
      ...
      {!projectDataEligibleForLinkingResponse ? (
        ...
        <Grid item container spacing={3}>
          ...
          <Grid item xs={12}>
            <JSONContainer data={projectDataEligibleForLinkingResponse} />
          </Grid>
        </Grid>
      )}
    </Grid>
...
export default ProjectDataEligibleForLinkingPage;
