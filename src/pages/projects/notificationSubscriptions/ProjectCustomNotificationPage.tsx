...
async function getProjectNotificationData(
  projectId: string,
  subId: any
): Promise<CustomNotificationSubscription> {
  // Generate axios parameter
  const ProjectParamCreator =
    ProjectCustomNotificationSubscriptionsApiAxiosParamCreator();
  const getProjectsParam =
    await ProjectParamCreator.getNotificationSubscription(projectId, subId);
...
function ProjectNotificationPage() {
  const [ProjectNotificationResponse, setProjectNotificationResponse] =
    useState<CustomNotificationSubscription | null>();
